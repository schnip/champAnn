...
mask = zeros(size(red));
mask(blue > 250 & red < 30 & green < 30) = 1; % blue health bar
mask(green > 250 & red < 30 & blue < 30) = 1; % green health bar
mask(red > 250 & blue < 30 & green < 30) = 1; % red health bar
mask(blue > 250 & red > 250 & green > 250) = 1; % black/empty health bar
...
for i=1:max(max(label))
    ...
    if (numPixels > 400 && numPixels < 500)
        ...
        healthCols = colNums(healthRows > 0);
        width = max(healthCols) - min(healthCols);
        
        if (height < 20 && width < 150)
